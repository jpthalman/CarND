...
class ConvNet(object):
    ...
    def train(self, train, val, threshold=0.99, save_loc='Checkpoints/model.ckpt',
        ...
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
                   self.LOGITS,
                   self.LABELS))
        optimizer = OPTIMIZER(self.LEARNING_RATE).minimize(loss)
                ...
                for batch_x, batch_y in self._batches(X_train, y_train):
                    sess.run(optimizer, 
                             feed_dict={self.DATA: batch_x, 
                                        self.LABELS: batch_y,
                                        self.KEEP_PROB[0]: self.KEEP_PROB[1]})
    ...
    def dropout(self):
        ...
        self.LOGITS = tf.nn.dropout(self.LOGITS, self.KEEP_PROB[0])
